...
func (h *handler) persistDocAndPages(newDoc document) (blob.Ref, error) {
	...
	for _, page := range newDoc.pages {
		if err := h.setAttribute(page, "document", newDoc.permanode.String()); err != nil {
			return br, fmt.Errorf("could not update scan %v with %v:%v", page, "document", newDoc.permanode)
		}
	}
	return pn, nil
}
