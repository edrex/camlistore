...
func newHandler() (*handler, error) {
	...
	userpass := strings.Split(config.Auth, ":")
	var am auth.AuthMode
	// TODO(mpl): maybe later offload that work to the auth package
	switch len(userpass) {
	case 2:
		if userpass[0] != "devauth" {
			return nil, fmt.Errorf("invalid devauth string syntax: %q", config.Auth)
		}
		am = &auth.DevAuth{
			Password: userpass[1],
		}
	case 3:
		if userpass[0] != "userpass" {
			return nil, fmt.Errorf("invalid auth string syntax. got %q, want \"userpass:username:password\"", config.Auth)
		}
